...
public class CustomerInvoice extends JPanel{ 
    JLabel jlb_date,jlb_mobile; 
    JDateChooser jdc_date;
    ...
    public CustomerInvoice() { 
        ...
        jdc_date = new JDateChooser();jdc_date.setBounds(100 - 50,50,200,25); 
...
//        add(jtf_date); 
        ...
        jb_order.addActionListener((ActionEvent ae)->{ 
            ...
            String date = ((JTextField)jdc_date.getDateEditor().getUiComponent()).getText();
            // GET DETAILS FROM SQL STORE INTO VALUES ARRAY 
            int row = 0;
            try{
                PreparedStatement pst;
                Connection con;
                ResultSet rs;
                String sql = "select o.orderid,s.stkname,s.quantity from order_tab o inner join order_det s on o.orderid = s.orderid where o.orderdt = '" + date + "' and o.custid = '" + jtf_mobile.getText() + "' order by orderid";
                con = DriverConnection.getConnection();
                pst = con.prepareStatement(sql);
                rs = pst.executeQuery();
                while(rs.next()){
